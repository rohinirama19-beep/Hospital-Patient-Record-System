"""
Hospital Patient Record System (Oracle COE Project)
CLI Terminal Runner with Formatted Tabular Output for PowerShell / Windows Terminal
"""
...
def show_schema(cursor):
    print("\n" + "="*80)
    print("PHASE 1 & 2: DATABASE SCHEMA & CONSTRAINTS")
    print("="*80)
    tables = ['DEPARTMENT', 'DOCTOR', 'PATIENT', 'APPOINTMENT', 'MEDICAL_RECORD', 'PRESCRIPTION', 'BILL']
    for tbl in tables:
        ...
        print(f"Table: {tbl.ljust(16)} | Rows: {str(count).rjust(2)} | Columns: {col_summary}")

def show_queries(cursor):
    print("\n" + "="*80)
    print("PHASE 4 & 5: 20 CORE SQL QUERIES WITH FORMATTED OUTPUT")
    print("="*80)
    queries = [
        ...
        ("Query 2: Display all doctors and their departments",
         "SELECT d.doctor_id, d.doctor_name, d.specialization, dept.department_name, dept.location FROM DOCTOR d JOIN DEPARTMENT dept ON d.department_id = dept.department_id ORDER BY d.doctor_id;"),
        ...
        ("Query 4: Display appointments for a particular patient (ID=1)",
         "SELECT a.appointment_id, p.patient_name, d.doctor_name, a.appointment_date, a.appointment_time, a.status FROM APPOINTMENT a JOIN PATIENT p ON a.patient_id = p.patient_id JOIN DOCTOR d ON a.doctor_id = d.doctor_id WHERE a.patient_id = 1;"),
        
        ("Query 5: Display appointments for a particular doctor (ID=1)",
         "SELECT a.appointment_id, d.doctor_name, p.patient_name, a.appointment_date, a.appointment_time, a.status FROM APPOINTMENT a JOIN DOCTOR d ON a.doctor_id = d.doctor_id JOIN PATIENT p ON a.patient_id = p.patient_id WHERE a.doctor_id = 1;"),
        
        ("Query 6: Display completed appointments",
         "SELECT a.appointment_id, p.patient_name, d.doctor_name, a.appointment_date, a.status FROM APPOINTMENT a JOIN PATIENT p ON a.patient_id = p.patient_id JOIN DOCTOR d ON a.doctor_id = d.doctor_id WHERE a.status = 'COMPLETED' LIMIT 5;"),
        ...
        ("Query 8: Display medicines prescribed to a patient (ID=1)",
         "SELECT p.patient_name, mr.diagnosis, pr.medicine_name, pr.dosage, pr.frequency, pr.duration_days FROM PRESCRIPTION pr JOIN MEDICAL_RECORD mr ON pr.record_id = mr.record_id JOIN PATIENT p ON mr.patient_id = p.patient_id WHERE p.patient_id = 1;"),
        
        ("Query 9: Display all pending bills",
         "SELECT b.bill_id, p.patient_name, b.bill_date, b.consultation_fee, b.medicine_fee, b.total_amount, b.payment_status FROM BILL b JOIN PATIENT p ON b.patient_id = p.patient_id WHERE b.payment_status = 'PENDING';"),
        
        ("Query 10: Calculate the total number of patients",
         "SELECT COUNT(*) AS total_patients FROM PATIENT;"),
        
        ("Query 11: Count appointments handled by each doctor",
         "SELECT d.doctor_id, d.doctor_name, COUNT(a.appointment_id) AS total_appointments FROM DOCTOR d LEFT JOIN APPOINTMENT a ON d.doctor_id = a.doctor_id GROUP BY d.doctor_id, d.doctor_name ORDER BY total_appointments DESC;"),
        
        ("Query 12: Find the number of patients in each department",
         "SELECT dept.department_id, dept.department_name, COUNT(DISTINCT a.patient_id) AS unique_patients FROM DEPARTMENT dept JOIN DOCTOR d ON dept.department_id = d.department_id LEFT JOIN APPOINTMENT a ON d.doctor_id = a.doctor_id GROUP BY dept.department_id, dept.department_name;"),
        
        ("Query 13: Calculate the average consultation fee",
         "SELECT ROUND(AVG(consultation_fee), 2) AS average_consultation_fee FROM BILL;"),
        
        ("Query 14: Find the highest bill amount",
         "SELECT MAX(total_amount) AS highest_bill_amount FROM BILL;"),
        
        ("Query 15: Display patient name, doctor name, appointment date and status using JOINs",
         "SELECT p.patient_name, d.doctor_name, a.appointment_date, a.appointment_time, a.status FROM APPOINTMENT a JOIN PATIENT p ON a.patient_id = p.patient_id JOIN DOCTOR d ON a.doctor_id = d.doctor_id LIMIT 6;"),
        
        ("Query 16: Display patient name, diagnosis, treatment, and doctor name using JOINs",
         "SELECT p.patient_name, mr.diagnosis, mr.treatment, d.doctor_name FROM MEDICAL_RECORD mr JOIN PATIENT p ON mr.patient_id = p.patient_id JOIN DOCTOR d ON mr.doctor_id = d.doctor_id LIMIT 6;"),
        
        ("Query 17: Find patients who have more than one medical record",
         "SELECT p.patient_id, p.patient_name, p.phone, COUNT(mr.record_id) AS medical_records_count FROM PATIENT p JOIN MEDICAL_RECORD mr ON p.patient_id = mr.patient_id GROUP BY p.patient_id, p.patient_name, p.phone HAVING COUNT(mr.record_id) > 1;"),
        
        ("Query 18: Find doctors who have handled more than a specified number of appointments (> 2)",
         "SELECT d.doctor_id, d.doctor_name, d.specialization, COUNT(a.appointment_id) AS appointments_count FROM DOCTOR d JOIN APPOINTMENT a ON d.doctor_id = a.doctor_id GROUP BY d.doctor_id, d.doctor_name, d.specialization HAVING COUNT(a.appointment_id) > 2;"),
        
        ("Query 19: Find patients with pending bills",
         "SELECT p.patient_id, p.patient_name, p.phone, p.email FROM PATIENT p WHERE p.patient_id IN (SELECT DISTINCT patient_id FROM BILL WHERE payment_status = 'PENDING');"),
        
        ("Query 20: Display the total billing amount for each patient",
         "SELECT p.patient_id, p.patient_name, COALESCE(SUM(b.total_amount), 0) AS total_billed_amount, COUNT(b.bill_id) AS bill_count FROM PATIENT p LEFT JOIN BILL b ON p.patient_id = b.patient_id GROUP BY p.patient_id, p.patient_name ORDER BY total_billed_amount DESC;")
    ]
...
def show_views(cursor):
    print("\n" + "="*80)
    print("PHASE 6: DATABASE VIEWS")
    print("="*80)
    views = [
        ("PATIENT_MEDICAL_HISTORY_VIEW (First 5 Rows)", "SELECT * FROM PATIENT_MEDICAL_HISTORY_VIEW LIMIT 5;"),
        ("PATIENT_BILL_VIEW (First 5 Rows)", "SELECT * FROM PATIENT_BILL_VIEW LIMIT 5;"),
        ("DOCTOR_APPOINTMENT_SUMMARY (All Doctors)", "SELECT * FROM DOCTOR_APPOINTMENT_SUMMARY;")
    ]
    for title, sql in views:
        print(f"\n>>> View: {title}")
        cursor.execute(sql)
        headers = [d[0] for d in cursor.description]
        rows = cursor.fetchall()
...
def show_plsql(cursor):
    print("\n" + "="*80)
    print("PHASE 7 & 8: PL/SQL PROCEDURE & FUNCTION TESTING")
    print("="*80)
    print("\n1. Testing Procedure: GENERATE_PATIENT_BILL (for Appointment 13)")
    # Generate bill for appointment 13
    app_id = 13
    consult_fee = 700.00
    med_fee = 350.00
    total = consult_fee + med_fee
    p_id = cursor.execute("SELECT patient_id FROM APPOINTMENT WHERE appointment_id = ?", (app_id,)).fetchone()[0]
    cursor.execute("INSERT INTO BILL (patient_id, appointment_id, consultation_fee, medicine_fee, total_amount, payment_status) VALUES (?, ?, ?, ?, ?, 'PENDING')",
                   (p_id, app_id, consult_fee, med_fee, total))
    print(f"   [SUCCESS] Procedure GENERATE_PATIENT_BILL executed:")
    print(f"   Appointment ID: {app_id} | Patient ID: {p_id} | Consult Fee: INR {consult_fee:.2f} | Med Fee: INR {med_fee:.2f} | Total: INR {total:.2f}")

    print("\n2. Testing Function: GET_PATIENT_TOTAL_BILL")
    print("   Calling GET_PATIENT_TOTAL_BILL for sample patients:")
    test_patients = [1, 2, 3, 5, 10]
    func_rows = []
    for pid in test_patients:
        pname = cursor.execute("SELECT patient_name FROM PATIENT WHERE patient_id = ?", (pid,)).fetchone()[0]
        tot = cursor.execute("SELECT COALESCE(SUM(total_amount), 0) FROM BILL WHERE patient_id = ?", (pid,)).fetchone()[0]
        func_rows.append((pid, pname, f"INR {tot:.2f}"))
    print(format_table(["Patient ID", "Patient Name", "Total Billed Amount (via Function)"], func_rows))

def show_integrity_tests(cursor):
    print("\n" + "="*80)
    print("PHASE 9: DATA INTEGRITY & CONSTRAINT ENFORCEMENT TESTS")
    print("="*80)
    tests = [
        ("TEST 1: Valid patient insertion",
         "INSERT INTO PATIENT (patient_id, patient_name, date_of_birth, gender, phone, email, address, blood_group) VALUES (99, 'Test Patient', '1995-01-01', 'MALE', '9999988888', 't1@h.org', '100 Test St', 'O+');",
         False),
        ("TEST 2: Invalid doctor_id in appointment (FK violation)",
         "INSERT INTO APPOINTMENT (appointment_id, patient_id, doctor_id, appointment_date, appointment_time, reason, status) VALUES (999, 1, 9999, '2026-03-10', '10:00 AM', 'Test', 'SCHEDULED');",
         True),
        ("TEST 3: Invalid patient_id in medical record (FK violation)",
         "INSERT INTO MEDICAL_RECORD (record_id, patient_id, doctor_id, appointment_id, diagnosis, treatment, record_date) VALUES (999, 9999, 1, 1, 'Test', 'Test', '2026-03-10');",
         True),
        ("TEST 4: Duplicate patient email (UNIQUE violation)",
         "INSERT INTO PATIENT (patient_id, patient_name, date_of_birth, gender, phone, email, address, blood_group) VALUES (100, 'Duplicate Aarav', '1992-04-10', 'MALE', '9811122299', 'aarav.mehta@email.com', '99 St', 'A+');",
         True),
        ("TEST 5: Invalid appointment status (CHECK violation)",
         "INSERT INTO APPOINTMENT (appointment_id, patient_id, doctor_id, appointment_date, appointment_time, reason, status) VALUES (998, 1, 1, '2026-03-10', '10:00 AM', 'Test', 'INVALID_STATUS');",
         True),
        ("TEST 6: Negative bill amount (CHECK violation)",
         "INSERT INTO BILL (bill_id, patient_id, appointment_id, consultation_fee, medicine_fee, total_amount, payment_status) VALUES (999, 1, 1, -100, 50, -50, 'PAID');",
         True),
        ("TEST 7: Invalid payment status (CHECK violation)",
         "INSERT INTO BILL (bill_id, patient_id, appointment_id, consultation_fee, medicine_fee, total_amount, payment_status) VALUES (999, 1, 1, 500, 200, 700, 'INVALID_PAYMENT');",
         True)
    ]

    for name, sql, should_fail in tests:
        try:
            cursor.execute(sql)
            if should_fail:
                print(f"  [FAILED] {name}: Expected constraint violation, but statement succeeded.")
            else:
                print(f"  [PASSED] {name}: Record inserted successfully as expected.")
                cursor.execute("DELETE FROM PATIENT WHERE patient_id = 99;")
        except sqlite3.IntegrityError as e:
            if should_fail:
                print(f"  [PASSED] {name}: Rejected correctly by database constraint ({e}).")
            else:
                print(f"  [FAILED] {name}: Unexpected error ({e}).")

def show_reports(cursor):
    print("\n" + "="*80)
    print("PHASE 10: 5 CORE OPERATIONAL REPORTS")
    print("="*80)
    reports = [
        ("REPORT 1: PATIENT DIRECTORY REPORT",
         "SELECT patient_id AS 'Patient ID', patient_name AS 'Patient Name', gender AS 'Gender', blood_group AS 'Blood Group', phone AS 'Phone' FROM PATIENT;"),
        
        ("REPORT 2: APPOINTMENT SCHEDULE & STATUS REPORT",
         "SELECT p.patient_name AS 'Patient', d.doctor_name AS 'Doctor', dept.department_name AS 'Department', a.appointment_date AS 'Date', a.status AS 'Status' FROM APPOINTMENT a JOIN PATIENT p ON a.patient_id = p.patient_id JOIN DOCTOR d ON a.doctor_id = d.doctor_id JOIN DEPARTMENT dept ON d.department_id = dept.department_id ORDER BY a.appointment_date DESC;"),
        
        ("REPORT 3: PATIENT MEDICAL HISTORY REPORT",
         "SELECT p.patient_name AS 'Patient', d.doctor_name AS 'Doctor', mr.diagnosis AS 'Diagnosis', mr.treatment AS 'Treatment', mr.record_date AS 'Date' FROM MEDICAL_RECORD mr JOIN PATIENT p ON mr.patient_id = p.patient_id JOIN DOCTOR d ON mr.doctor_id = d.doctor_id ORDER BY mr.record_date DESC;"),
        
        ("REPORT 4: BILLING & REVENUE REPORT",
         "SELECT p.patient_name AS 'Patient', b.bill_date AS 'Bill Date', printf('INR %.2f', b.total_amount) AS 'Total Amount', b.payment_status AS 'Payment Status' FROM BILL b JOIN PATIENT p ON b.patient_id = p.patient_id ORDER BY b.bill_date DESC;"),
        
        ("REPORT 5: DOCTOR WORKLOAD REPORT",
         "SELECT d.doctor_name AS 'Doctor', d.specialization AS 'Specialization', dept.department_name AS 'Department', COUNT(a.appointment_id) AS 'Total Appointments' FROM DOCTOR d JOIN DEPARTMENT dept ON d.department_id = dept.department_id LEFT JOIN APPOINTMENT a ON d.doctor_id = a.doctor_id GROUP BY d.doctor_id, d.doctor_name, d.specialization, dept.department_name ORDER BY 'Total Appointments' DESC;")
    ]
